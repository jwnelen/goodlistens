...
function App() {
  ...
  const [getPodcasts, { loading, error, data }] = useLazyQuery(SEARCH_EPISODES);
  let episodes;
  let podcasts;

  if (data?.searchForTerm) {
    console.log(data);
    episodes = data?.searchForTerm?.podcastEpisodes;
    podcasts = data?.searchForTerm?.podcastSeries;
  }
